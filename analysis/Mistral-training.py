...
__description__ = "train FSoLS-24-v2 datatset with Mistral 7B"
...
EPOCH_AMOUNT = 5
OUTPUT_FILENAME = "models/FSoLS-24-v2_Mistral7b_512tokenz_e5_lr3e-5_mlclass"
token = "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"
#date = 2024-08-09
...
def evaluate_model(model, val_inputs, val_masks, val_labels):
    ...
    if len(all_logits) == 0:
        raise ValueError("No logits were generated. Please check the input data and model.")
...
def main():
    ...
    train_inputs, val_inputs,  train_masks,val_masks,train_labels, val_labels = split_train_val_data(tokens, split_ratio, labels_onehot)
    ...
    train_inputs, val_inputs = torch.tensor(train_inputs).to(device), torch.tensor(val_inputs).to(device)
    train_masks, val_masks = torch.tensor(train_masks).to(device), torch.tensor(val_masks).to(device)
    ...
    # allocate model to weights and biases
    wandb.watch(model)
    ...
    for epoch in range(epochs):
        ...
        if use_wandb:
            ...
            filename = f"models/mistral7b_t512_e{epoch+1}_lr3e-5_mlclass"
            model.save_pretrained(filename)


    model.save_pretrained(OUTPUT_FILENAME)
    print("done")
