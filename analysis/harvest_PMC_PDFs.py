...
def download_pdf(complete_link, i):
    ...
    path = "dummy_science_PMC-PDF.pdf"
    pdf = open(path, "wb")
...
def main():
    ...
    # complete_links_df = pd.DataFrame(complete_links)
    # complete_links_df.to_csv('data/science_PMC_complete_url_list_2023-09-17.csv' , mode ='a', index=False, header=False)

    # loop through each document-url
    # with open('data/science_PMC_complete_url_list_2023-09-17.csv') as file_obj:
    with open("science_PMC_complete_url_list_2023-09-17.csv") as file_obj:

        reader_obj = csv.reader(file_obj)
        for complete_link in reader_obj:
            i += 1
            print("Processing file:", i)
            complete_link = complete_link[0]
            ...
            df_all = compile_infos(cleaned_txt, df_all, text_id, complete_link, i)

            df_all.to_csv(
                "scientific_PMC-PDF_basic-data_2023-09-21.csv",
                mode="w",
                index=False,
                header=True,
            )
